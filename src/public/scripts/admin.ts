(() => {
  ...
  if (contentForm) {
    ...
    function buildSeoIssues() {
      const issues = [];
      const addIssue = (message, selector) => issues.push({ message, selector });
      ...
      const orderNumbers = sectionOrderFields
      ...
      const urlChecks = [
        { selector: "[name='brand_instagram']", label: "Ссылка на Инстаграм", allowTel: false },
        { selector: "[name='brand_order_link']", label: "Ссылка на заказ", allowTel: true },
        { selector: "[name='brand_map_2gis']", label: "Ссылка 2GIS", allowTel: false },
        { selector: "[name='brand_map_yandex']", label: "Ссылка Яндекс Карты", allowTel: false },
        { selector: "[name='hero_primary_url']", label: "Hero: URL кнопки 1", allowTel: true },
        { selector: "[name='hero_secondary_url']", label: "Hero: URL кнопки 2", allowTel: true }
      ];
      ...
      for (const item of urlChecks) {
        ...
        if (!isValidUrlLike(value, { allowTel: item.allowTel })) {
      ...
      const sectionKeys = ["services", "process", "materials", "about", "gallery", "contacts"];
      ...
      if (mediaIdsField && picker && addButton) {
        addButton.addEventListener("click", () => {
          ...
          if (!existing.includes(value)) {
            ...
            mediaIdsField.dispatchEvent(new Event("input", { bubbles: true }));
          }
        });
      }
    });
